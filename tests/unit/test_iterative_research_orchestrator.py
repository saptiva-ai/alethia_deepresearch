...
from domain.services.iterative_research_svc import (
...
class TestIterativeResearchOrchestrator:
    ...
    async def test_execute_deep_research_single_iteration_success(self, mock_get_logger):
        ...
        # Mock the services
        orchestrator.planner.create_plan = Mock(return_value=ResearchPlan(
            main_query="test query",
            sub_tasks=[
                ResearchSubTask(id="task1", query="subquery1", sources=["web"])
            ]
        ))
    ...
    async def test_execute_deep_research_multi_iteration(self, mock_get_logger):
        ...
        mock_evidence_iter1 = [Evidence(
            id="ev1", 
            source=EvidenceSource(url="http://test1.com", title="Test1", fetched_at=datetime.utcnow()),
            excerpt="Evidence 1", hash="hash1", tool_call_id="call1", score=0.8, tags=["test"]
        )]
        
        mock_evidence_iter2 = [Evidence(
            id="ev2",
            source=EvidenceSource(url="http://test2.com", title="Test2", fetched_at=datetime.utcnow()), 
            excerpt="Evidence 2", hash="hash2", tool_call_id="call2", score=0.9, tags=["test"]
        )]
        ...
        mock_refinement_queries = [
            RefinementQuery(
                query="additional research query",
                gap_addressed="market_data",
                priority=3,
    ...
    async def test_execute_deep_research_max_iterations_reached(self, mock_get_logger):
        ...
        orchestrator = IterativeResearchOrchestrator(
            max_iterations=2,
            min_completion_score=0.9  # High threshold
        )
        
        # Mock services
        orchestrator.planner.create_plan = Mock(return_value=ResearchPlan(
            main_query="test query",
            sub_tasks=[ResearchSubTask(id="task1", query="subquery1", sources=["web"])]
        ))
        
        mock_evidence = [Evidence(
            id="ev1",
            source=EvidenceSource(url="http://test.com", title="Test", fetched_at=datetime.utcnow()),
            excerpt="Limited evidence", hash="hash1", tool_call_id="call1", score=0.7, tags=["test"]
        )]
    ...
    async def test_execute_refinement_queries_parallel_with_queries(self):
        ...
        refinement_queries = [
            RefinementQuery(
                query="refinement query 1",
                gap_addressed="market_data",
                priority=5,
                expected_sources=["web", "news"]
            ),
            RefinementQuery(
                query="refinement query 2", 
                gap_addressed="competitive_analysis",
                priority=4,
        ...
        mock_evidence = [Evidence(
            ...
            excerpt="Refined evidence", hash="ref_hash", tool_call_id="ref_call", score=0.9, tags=["refined"]
        )]
    ...
    def test_get_research_summary(self):
        ...
        gaps = [
            InformationGap(
                gap_type="competitor_data",
                description="Missing competitor analysis",
                priority=5,
                suggested_query="Find main competitors"
            ),
            InformationGap(
                gap_type="market_size",
                description="Market size data missing",
                priority=4,
                suggested_query="Research market size data"
            )
        ]
...
class TestIterativeResearchOrchestratorIntegration:
    ...
    async def test_full_workflow_with_event_logging(self, mock_get_logger):
        ...
        orchestrator.planner.create_plan = Mock(return_value=ResearchPlan(
            main_query="integration test",
            sub_tasks=[ResearchSubTask(id="t1", query="sub1", sources=["web"])]
        ))
        
        orchestrator.researcher.execute_plan_parallel = AsyncMock(return_value=[])
        
        orchestrator.evaluator.evaluate_research_completeness = Mock(return_value=CompletionScore(
            overall_score=0.8, completion_level=CompletionLevel.ADEQUATE,
            coverage_areas={}, identified_gaps=[], confidence=0.8, reasoning="Test"
        ))
