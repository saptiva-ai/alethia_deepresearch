...
class LoadTestRunner:
    ...
    async def run_health_check_load_test(self) -> LoadTestResults:
        ...
        async def make_health_request(session: httpx.AsyncClient, user_id: int) -> RequestResult:
    ...
    async def run_stress_test(self) -> LoadTestResults:
        ...
        async def make_stress_request(session: httpx.AsyncClient, user_id: int) -> RequestResult:
    ...
    async def _run_concurrent_test(self, request_func, test_name: str) -> LoadTestResults:
        ...
        async with httpx.AsyncClient() as session:
            ...
            for user_id in range(self.config.concurrent_users):
                ...
                tasks.append(
                    self._user_session(session, user_id, user_requests, request_func)
                )
            ...
            for user_id in range(self.config.concurrent_users):
                ...
                tasks.append(
                    self._delayed_user_session(
                        session, user_id, requests_per_user, request_func, start_delay
                    )
                )
    ...
    async def _user_session(self, session: httpx.AsyncClient, user_id: int, 
                           num_requests: int, request_func) -> List[RequestResult]:
    ...
    async def _delayed_user_session(self, session: httpx.AsyncClient, user_id: int,
                                  num_requests: int, request_func, delay: float) -> List[RequestResult]:
    ...
    def _analyze_results(self, results: List[RequestResult], start_time: datetime, 
                        end_time: datetime, test_name: str) -> LoadTestResults:
